...
List<(String, String, int)> getWeeksOfMonth(DateTime inputDate) {
  ...
  final lastDayOfMonth = DateTime(year, month + 1, 1).subtract(const Duration(days: 1));

  // 找到当前月的第一个周一（若首日不是周一，则向前推算）
  final firstDayWeekday = firstDayOfMonth.weekday;
  final daysToFirstMonday = (DateTime.monday - firstDayWeekday + 7) % 7;
  DateTime currentMonday = firstDayOfMonth.add(Duration(days: daysToFirstMonday));

  final List<(String, String, int)> weeks = [];

  // 遍历每周，直到超出月末
  while (currentMonday.month == month || currentMonday.isBefore(lastDayOfMonth)) {
    // 若当前周一已跨月，终止循环
    ...
    // 计算年度周数（ISO 8601标准）
    final weekNumber = int.parse(DateFormat('w').format(startDate));
