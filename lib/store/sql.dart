...
class DBManager {
  ...
  Future<bool> openDB(String path) async{
    ...
    db = await openDatabase(dbPath, version: 1, onCreate: (Database db, int version){
      ...
      db.execute(dreamTable).then((_) {
        db.rawInsert("INSERT INTO category(name,type,icon,sort_num,updated_at) VALUES"
      ...
      db.execute(payTable).then((_) {
        db.rawInsert("INSERT INTO `pay_platform`(`pay_name`, `pay_icon`) VALUES('支付宝', 'alipay'),('微信', 'wechat'),('银联', 'unionpay'),('其他', 'other')");
      });
  ...
  Future<int> insertRecord(RecordItem data) async {
    ...
    return db.transaction((txn) async {
      String dataStr = "INSERT INTO `records`("
          ...
          "updated_at"
          ") VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?)";
      return txn.rawInsert(dataStr, [data.amount, data.name, data.categoryId, data.categoryType, billYear, billMonth, data.billDate.toIso8601String(), data.remark, DateTime.now().toIso8601String()]);
    });
  }
  /// 更新记录项目
  Future<int> updateRecord(RecordItem data) async {
    var billYear = data.billDate.year;
    var billMonth = data.billDate.month;
    String sqlData = "UPDATE `records` SET amount=?, name=?, category_id=?,bill_year=? bill_month=?, bill_date=?, remark=?, updated_at=? WHERE dreamId=?";
    int count = await db.rawUpdate(sqlData, [data.amount, data.name, data.categoryId, billYear, billMonth, data.billDate.toIso8601String(), data.remark, DateTime.now().toIso8601String(), data.id]);
    return count;
  }
  /// 查询记录列表
  Future<List<RecordItem>> selectRecordList([CategoryType categoryType = CategoryType.expense, int page = 1, int? billYear, int? billMonth, String order = 'bill_date', String orderType = 'DESC']) async {
    var limitStart = getPageLimitStart(page);
    String data = "SELECT r.*, c.icon FROM `records` r LEFT JOIN category c ON r.category_id = c.id WHERE category_type = ?";
  ...
  Future<RecordDetail?> selectRecordById(int id) async {
    String data = "SELECT r.*, c.icon, p.pay_name, p.pay_icon FROM `records` r LEFT JOIN category c ON r.category_id = c.id LEFT JOIN pay_platform p ON r.pay_platform_id = p.id WHERE r.id = ?";
    List<Map<String, dynamic>> list = await db.rawQuery(data, [id]);
    if (list.isEmpty){
      return null;
    }
    //将查询到的数据映射成模型
    List<RecordDetail> models = list.map((e) => RecordDetail.fromJson(e)).toList();
    return models.first;
  }
  /// 删除记录
  ...
  /// 获取列表
  Future<List<CategoryItemProvider>> queryCategoryList([CategoryType type = CategoryType.expense]) async {
    String data = "SELECT * FROM `category` WHERE type = ? AND is_deleted = 0 ORDER BY `sort_num` ASC";
    List<Map<String, dynamic>> list = await db.rawQuery(data, [type.state]);
    if (list.isEmpty){
      return [];
    }
    // return list;
    List<CategoryItemProvider> models = list.map((e) => CategoryItemProvider.fromJson(e)).toList();
    return models;
  }
  /// 更新分类
  Future<int> updateCategory(CategoryItemProvider data) async {
    String sqlData = "UPDATE `category` SET name=?, icon=?, sort_num=?, updated_at=? WHERE id=?";
    return await db.rawUpdate(sqlData, [data.name, data.icon, data.sortNum, DateTime.now().toIso8601String(), data.id]);
  }
  /// 删除分类
  ...
  Future<int> insertCategory(CategoryItemProvider data) async {
    String sqlData = "INSERT INTO `category`("
        ...
        "updated_at"
        ") VALUES(?,?,?,?,?)";
    return await db.rawInsert(sqlData, [data.name, data.icon, data.type.state, data.sortNum, DateTime.now().toIso8601String()]);
  }
}
