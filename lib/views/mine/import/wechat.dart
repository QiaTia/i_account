...
class ImportWechat extends ConsumerStatefulWidget {
  const ImportWechat({super.key});
  @override
  ConsumerState<ImportWechat> createState() => _ImportWechatState();
}

class _ImportWechatState extends ConsumerState<ImportWechat> {
  ...
  void onFileImport() async {
    ...
    importLocalFileParseByWechat().then((list) {
      if (list.isNotEmpty) {
        setState(() {
          this.list = list;
        });
